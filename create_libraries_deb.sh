#!/bin/bash

set -e

source ./setup_environment.sh

# 创建单个库的 DEB 包
create_library_deb() {
    local lib_name=$1
    local version=$2
    local description=$3
    local dependencies=$4
    local system_pkg_name=${5:-"lib$lib_name"}  # 添加系统标准包名参数，默认为lib+库名

    log_section "Creating $lib_name DEB package"

    local install_dir="$CROSS_BASE/install/$lib_name"
    local pkg_dir="$CROSS_BASE/packages/${system_pkg_name}_${version}_armel"

    # 检查安装目录是否存在
    if [ ! -d "$install_dir" ]; then
        log_error "$lib_name install directory $install_dir does not exist!"
        return 1
    fi
    
    # 清理并创建包目录结构
    log_info "Creating package directory structure..."
    rm -rf "$pkg_dir"
    mkdir -p "$pkg_dir/DEBIAN"
    
    # 复制库文件到包目录，保持目录结构
    log_info "Copying library files to package directory..."
    if [ -d "$install_dir" ]; then
        # 创建 usr 目录结构
        mkdir -p "$pkg_dir/usr"
        
        # 复制文件，但排除一些不需要的文件
        rsync -av \
            --exclude='pkgconfig' \
            --exclude='share/info/dir' \
            "$install_dir/" "$pkg_dir/usr/"
        
        # 如果有 pkgconfig 文件，放到正确位置
        if [ -d "$install_dir/lib/pkgconfig" ]; then
            mkdir -p "$pkg_dir/usr/lib/arm-linux-gnueabi/pkgconfig"
            cp "$install_dir/lib/pkgconfig"/* "$pkg_dir/usr/lib/arm-linux-gnueabi/pkgconfig/" 2>/dev/null || true
        fi
        
        # 移动库文件到 multiarch 目录
        if [ -d "$pkg_dir/usr/lib" ] && [ "$(ls -A $pkg_dir/usr/lib)" ]; then
            log_info "Moving library files to multiarch directory..."
            mkdir -p "$pkg_dir/usr/lib/arm-linux-gnueabi"
            find "$pkg_dir/usr/lib" -maxdepth 1 -name "*.so*" -exec mv {} "$pkg_dir/usr/lib/arm-linux-gnueabi/" \;
            find "$pkg_dir/usr/lib" -maxdepth 1 -name "*.a" -exec mv {} "$pkg_dir/usr/lib/arm-linux-gnueabi/" \;
            find "$pkg_dir/usr/lib" -maxdepth 1 -name "*.la" -exec mv {} "$pkg_dir/usr/lib/arm-linux-gnueabi/" \;
            find "$pkg_dir/usr/lib" -maxdepth 1 -name "lib*" -type d -exec mv {} "$pkg_dir/usr/lib/arm-linux-gnueabi/" \;
        fi
    fi
    
    # 计算安装大小
    local installed_size=$(du -sk "$pkg_dir/usr" | cut -f1)
    
    # 创建控制文件
    log_info "Creating control file..."
    cat > "$pkg_dir/DEBIAN/control" << EOF
Package: ${system_pkg_name}
Version: ${version}
Section: libs
Priority: optional
Architecture: armel
Maintainer: spamstech <hi@spams.tech>
Installed-Size: ${installed_size}
Description: ${description}
 Cross-compiled ${lib_name} library for ARM architecture (armel).
 This package contains the shared libraries and development files.
EOF
    
    # 如果有依赖关系，添加到控制文件
    if [ -n "$dependencies" ]; then
        echo "Depends: $dependencies" >> "$pkg_dir/DEBIAN/control"
    fi
    
    # 创建 postinst 脚本
    log_info "Creating postinst script..."
    cat > "$pkg_dir/DEBIAN/postinst" << 'EOF'
#!/bin/bash
set -e
if [ "$1" = "configure" ]; then
    ldconfig
fi
EOF
    chmod 755 "$pkg_dir/DEBIAN/postinst"
    
    # 创建 postrm 脚本
    log_info "Creating postrm script..."
    cat > "$pkg_dir/DEBIAN/postrm" << 'EOF'
#!/bin/bash
set -e
if [ "$1" = "remove" ] || [ "$1" = "purge" ]; then
    ldconfig
fi
EOF
    chmod 755 "$pkg_dir/DEBIAN/postrm"
    
    # 创建文件清单
    find "$pkg_dir/usr" -type f | sed "s|$pkg_dir||" > "$pkg_dir/DEBIAN/files.list"
    
    # 构建 DEB 包
    log_info "Building DEB package..."
    dpkg-deb -Zgzip --uniform-compression --build "$pkg_dir"
    
    log_success "Created: ${pkg_dir}.deb"

    # 验证包
    log_info "Package info:"
    dpkg-deb -I "${pkg_dir}.deb"
    echo ""
}

# 创建所有库的 DEB 包
log_section "Creating DEB packages for all libraries"

create_library_deb "zlib" "${ZLIB_VERSION}-1" "Compression library - runtime" "libc6" "zlib1g-spams"

create_library_deb "openssl" "${OPENSSL_VERSION}-1" "Secure Sockets Layer toolkit - runtime" "libc6" "libssl3-spams"

create_library_deb "libffi" "${LIBFFI_VERSION}-1" "Foreign Function Interface library runtime" "libc6" "libffi8-spams"

create_library_deb "sqlite" "${SQLITE_VERSION}-1" "SQLite 3 shared library" "libc6" "libsqlite3-0-spams"

create_library_deb "ncurses" "${NCURSES_VERSION}-1" "shared libraries for terminal handling" "libc6" "libncurses6-spams"

create_library_deb "readline" "${READLINE_VERSION}-1" "GNU readline and history libraries, runtime" "libc6, libncurses6-spams" "libreadline8-spams"

create_library_deb "bzip2" "${BZIP2_VERSION}-1" "high-quality block-sorting file compressor library - runtime" "libc6" "libbz2-1.0-spams"

create_library_deb "xz" "${XZ_VERSION}-1" "XZ-format compression library" "libc6" "liblzma5-spams"

create_library_deb "gdbm" "${GDBM_VERSION}-1" "GNU dbm database routines (runtime version)" "libc6" "libgdbm6-spams"

create_library_deb "util-linux" "${UTIL_LINUX_VERSION}-1" "miscellaneous system utilities - runtime libraries" "libc6" "libuuid1-spams"

log_success "All DEB packages created successfully!"
log_info "Packages location: $CROSS_BASE/packages/"
ls -la "$CROSS_BASE/packages/"*.deb
